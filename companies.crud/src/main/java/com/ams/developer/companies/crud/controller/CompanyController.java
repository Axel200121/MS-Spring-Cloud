...
public class CompanyController {
    ...
    @Operation(summary = "save in BD a company given a company from body")
    @PostMapping
    ...
    @Operation(summary = "update in BD a company given a company from body")
    @PutMapping(path = "{name}")
    public ResponseEntity<Company> update(@RequestBody Company company, @PathVariable String name){
        log.info("PUT: company {}", name);
        return ResponseEntity.ok(this.companyService.update(name, company));
    }

    @Operation(summary = "Delete in BD a company given a company name")
